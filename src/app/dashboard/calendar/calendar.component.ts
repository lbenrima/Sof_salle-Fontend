...
export class CalendarComponent implements AfterViewInit {
    ...
    constructor(private calendarService: CalendarService, private caracsService: CaracsService, private config: Configuration,
    ...
    ngAfterViewInit() {
        ...
        var self = this;
        // console.log("console print calendar.component.ts");
    ...
    initCalendar() {
        ...
        this.calendarElement.fullCalendar({
            ...
            events: {
                url: Urlserveur + '/reservation/res?email=' + this.user.mail,
                type: 'GET',
                data: {
                ...
                if (event.mailReservedBy == window.localStorage.getItem("currentUserName")) {
                    ...
                    $('#txt').val(p);
                     if (event.simple == true) {
                         ...
                         $('#btndeleteRec').attr('childId', event.idchild);
                         $('#btndeleteRec').show();
                          $('#btndeleteSerie').show();
    ...
    rerenderFullCalendar() {
        ...
        //this.calendarElement.fullCalendar('rerenderEvents');
        //this.router.navigate(['/calendar']).catch(error => console.log(error));
        // $('#fullCalendar').fullCalendar('refresh');
        setTimeout(() => {
            this.calendarElement.fullCalendar('addEventSource', { "url": this.UrlServise + '/reservation/res?email=' + this.user.mail });
...
console.log("childId" + $('#btndeleteRec').attr('childId'));
    let childid = $('#btndeleteRec').attr('childId');
        swal({
            title: "Voulez vous supprimer cette réservation ?",
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            ...
            if (result) {
                this.calendarService.deleteOccurence(childid).subscribe(
                    data => {
                        $('#calendar').fullCalendar('removeEvents',childid);
                        this.rerenderFullCalendar();
                    },
                    error => console.log(error),
                );
